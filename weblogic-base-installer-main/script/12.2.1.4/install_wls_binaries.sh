...
if [ "$weblogic_installer" == "generic" ]; then
  msg "Installing WebLogic ${weblogic_version} binaries using the generic installer ..."
  installer_jar="fmw_${weblogic_version}.0_wls.jar"
  install_type="WebLogic Server"
elif [ "$weblogic_installer" == "infrastructure" ]; then
  msg "Installing WebLogic ${weblogic_version} binaries using the FMW infrastructure installer ..."
  installer_jar="fmw_${weblogic_version}.0_infrastructure.jar"
  install_type="Fusion Middleware Infrastructure"
else
  err "Invalid weblogic_installer."
  exit 1
fi
