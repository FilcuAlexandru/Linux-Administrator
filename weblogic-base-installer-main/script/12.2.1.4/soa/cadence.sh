######################################
# Cadence for SOA Suite installation #
######################################
msg "WebLogic Base Installer $wls_installer_version for WebLogic $weblogic_version with SOA Suite"
# TODO from here ...
# Check prerequisites
for host in "${MACHINES[@]}"; do
  dostep script/pre_check.sh $host
  dostep script/${weblogic_version}/pre_check.sh $host
  dostep script/${weblogic_version}/soa/soa_pre_check.sh $host
done
...
for host in "${MACHINES[@]}"; do
  ...
  dostep script/${weblogic_version}/soa/install_soa_binaries.sh $host
  dostep script/${weblogic_version}/install_wls_patches.sh $host
  dostep script/${weblogic_version}/soa/install_soa_patches.sh $host
  dostep script/install_wdt.sh $host
done

msg "----- S A F E   R E S T A R T   P O I N T -------"

# Run the Repository Creation Utility to create the SOA schemas
dostep script/${weblogic_version}/soa/create_soa_schemas.sh

# Create SOA Domain
dostep script/${weblogic_version}/soa/create_soa_domain.sh
...
dostep script/${weblogic_version}/soa/configure_soa.sh
...
dostep script/${weblogic_version}/soa/apply_reference_config.sh
